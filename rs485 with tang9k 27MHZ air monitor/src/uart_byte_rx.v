module uart_byte_rx(
	...
	input clk;    // Global module clock input, 50M
	input reset_n;   // Reset signal input, active low

	input [2:0]baud_set;  // Baud rate setting
	input uart_rx;   // UART input signal
	
	output [7:0]data_byte; // 1-byte data received via UART
	output rx_done;   // 1-byte data reception completion flag
	
	reg [7:0]data_byte;
	reg rx_done;
	reg uart_rx_sync1;   // Synchronization register
	reg uart_rx_sync2;   // Synchronization register

	reg uart_rx_reg1;    // Data register
	reg uart_rx_reg2;    // Data register

	reg [15:0]bps_DR;    // Maximum value for frequency division counter
	reg [15:0]div_cnt;   // Frequency division counter
	reg bps_clk;   // Baud rate clock	
	reg [7:0] bps_cnt;   // Baud rate clock counter	
	reg uart_state; // Data reception state
	...
	// Synchronize the serial input signal to eliminate metastability
