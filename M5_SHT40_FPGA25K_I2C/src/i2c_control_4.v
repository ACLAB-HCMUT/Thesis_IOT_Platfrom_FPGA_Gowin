...
//                    rddata <= Rx_DATA;
                ...
                WR_REG: begin
                    ...
                    case (cnt)
                        0: write_byte(WR | STA, 8'h88); // Device ID with Write Command
                        1: write_byte(WR|STO, 8'hFD);             // Data 1
...
//        0: write_byte(WR | STA, 8'h70);   // Device Address with Read Command
        0: write_byte(WR | STA, 8'h89);   // Read Command with ACK
//        1: read_byte(RD | ACK);                // Read state byte
        ...
        2: begin
            read_byte(RD | ACK);
            // hum[11:4] <= Rx_DATA;    
             temp[15:8] <= Rx_DATA;           // Read Humidity byte 2
        end
        3: begin
            read_byte(RD | ACK);
        ...
        6:begin read_byte(RD | NACK | STO);        // Read CRC byte and stop condition
             hum[7:0] <= Rx_DATA; 
        end
        default: ;
    endcase
...
//            7: begin cnt <= 8; state <= RD_REG; end
            6: begin state <= RD_REG_DONE; cnt <= 0; end
            default:;
        endcase
    end
end
