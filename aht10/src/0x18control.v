        ...
        if (!Rst_n) begin
            ...
            case (state)
                ...
                WR_REG: begin
                    state <= WAIT_WR_DONE;
                    case (cnt)
                        0: write_byte(WR | STA, 8'h70); // Device ID with Write Command
                        1: write_byte(WR, 8'hAC);             // Trigger Measurement Command
                        2: write_byte(WR, 8'h33);             // Data 0
                        3: write_byte(WR, 8'h00);             // Data 1
                        4: state <= WAIT_DLY;                 // After trigger, wait for delay
                        default: ;
                    endcase
                end
                ...
                WAIT_WR_DONE: begin
                    ...
                    if (Trans_Done) begin
                        ...
                        case (cnt)
                            ...
                            1: begin state <= WR_REG; cnt <= 2; end
                            2: begin state <= WR_REG; cnt <= 3; end
                            3: begin state <= WR_REG; cnt <= 4; end
                            4: state <= WR_REG_DONE;
                            default: state <= IDLE;
                        endcase
                    end
...
//        0: write_byte(WR | STA, 8'h70);   // Device Address with Read Command
        0: write_byte(WR | STA, 8'h71);   // Read Command with ACK
        1: read_byte(RD | ACK);                // Read state byte
        2: begin
            read_byte(RD | ACK);
            hum[19:12] <= Rx_DATA;            // Read Humidity byte 1
        end
        3: begin
            read_byte(RD | ACK);
            hum[11:4] <= Rx_DATA;             // Read Humidity byte 2
        end
        4: begin
            read_byte(RD | ACK);
            hum[3:0] <= Rx_DATA[7:4];
            temp[19:16] <= Rx_DATA[3:0];     // Read Temperature (first byte)
        end
        5: begin
            read_byte(RD | ACK);
            temp[15:8] <= Rx_DATA;           // Read Temperature (second byte)
        end
        6: begin
            read_byte(RD | ACK);
            temp[7:0] <= Rx_DATA;            // Read Temperature (third byte)
        end
        7: read_byte(RD | NACK | STO);        // Read CRC byte and stop condition
        default: ;
    endcase
end
...
//            7: begin cnt <= 8; state <= RD_REG; end
            7: begin state <= RD_REG_DONE; cnt <= 0; end
            default: state <= IDLE;
        endcase
    end
end
