            ...
            case (state)
                ...
                WR_REG: begin
                    ...
                    case (cnt)
                        ...
                        3: write_byte(WR|STO, 8'h00);             // Data 1
...
//        0: write_byte(WR | STA, 8'h70);   // Device Address with Read Command
        ...
        3: begin
            read_byte(RD | ACK);
            // hum[11:4] <= Rx_DATA;    
             hum[19:12] <= Rx_DATA;           // Read Humidity byte 2
        end
        4: begin
            read_byte(RD | ACK);

            // hum[3:0] <= Rx_DATA[7:4];
            // temp[19:16] <= Rx_DATA[3:0];     // Read Temperature (low byte),humid high byte
             hum[11:4] <= Rx_DATA;
        end
        5: begin
            read_byte(RD | ACK);
        ...
        7:begin read_byte(RD | NACK | STO);        // Read CRC byte and stop condition
            temp[7:0] <= Rx_DATA; 
        end
        default: ;
    endcase
